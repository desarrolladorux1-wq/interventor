(function () {
  ...
  async function exportarSupervisionPdf(evento) {
    ...
    if(tipo==='diario'){
      ...
      const borde=()=>{doc.setDrawColor(30);doc.setLineWidth(.18);};
      ...
      const texto=(valor,x,y,w,h,{tam=4.1,negrita=false,centrado=false,color=negro}={})=>{
        ...
        const lineas=doc.splitTextToSize(String(valor||'—'),Math.max(2,w-1.8)).slice(0,Math.max(1,Math.floor((h-1)/(tam*1.15))));
        const inicio=y+Math.max(tam+.7,(h-lineas.length*tam*1.15)/2+tam*.8);
      ...
      // Cabecera combinada
      rect(14,8,30,20);if(logoFise)doc.addImage(logoFise,'PNG',19,10,19,14,undefined,'FAST');else texto('FISE',14,8,30,20,{tam:8,negrita:true,centrado:true});
      celda(44,8,102,6,'FORMATO',{relleno:naranja,color:[255,255,255],negrita:true,centrado:true,tam:4.3});
      celda(44,14,102,14,'INFORME DIARIO DE TRABAJO - IDT',{negrita:true,centrado:true,tam:5.2});
      celda(146,8,50,6,'END-FPY-FI-04',{negrita:true,centrado:true,tam:3.8});
      celda(146,14,25,7,'Fecha',{centrado:true,tam:3.6});celda(171,14,25,7,fechaCorta,{centrado:true,tam:3.5});
      celda(146,21,25,7,'Versión',{centrado:true,tam:3.6});celda(171,21,25,7,'001',{centrado:true,tam:3.6});
      // Datos generales, con uniones equivalentes al formato END-FPY-FI-04.
      celda(14,31,11,9,'PROYECTO.',{relleno:gris,color:[255,255,255],negrita:true,centrado:true,tam:3.5});celda(25,31,82,9,valor('supervisionProyecto')||proyecto.nombre,{tam:3.6});
      celda(107,31,23,9,'N° INFORME',{relleno:gris,color:[255,255,255],negrita:true,centrado:true,tam:3.6});celda(130,31,66,9,valor('supervisionNumero'),{relleno:[240,255,0],tam:4.1});
      celda(14,40,11,6,'SUPERVISIÓN',{relleno:gris,color:[255,255,255],negrita:true,centrado:true,tam:3.1});celda(25,40,47,6,valor('supervisionSupervisor'),{tam:3.6});celda(72,40,16,6,'CONTRATISTA',{relleno:gris,color:[255,255,255],negrita:true,centrado:true,tam:3.1});celda(88,40,19,6,valor('supervisionContratista'),{tam:3.6});celda(107,40,23,6,'LUGAR DE SUPERVISIÓN',{relleno:gris,color:[255,255,255],negrita:true,centrado:true,tam:2.8});celda(130,40,66,6,valor('supervisionLugar'),{tam:3.1});
      celda(14,46,11,5,'CLIENTE',{relleno:gris,color:[255,255,255],negrita:true,centrado:true,tam:3.3});celda(25,46,47,5,valor('supervisionCliente'),{tam:3.1});celda(72,46,16,5,'HOJA',{relleno:gris,color:[255,255,255],negrita:true,centrado:true,tam:3.4});celda(88,46,19,5,'1 DE 3',{centrado:true,tam:3.5});celda(107,46,23,5,'FECHA DE EJECUCIÓN',{relleno:gris,color:[255,255,255],negrita:true,centrado:true,tam:2.9});celda(130,46,66,5,fechaCorta,{tam:3.3});
      celda(14,51,182,4,'ESPECIALIDAD CIVIL - MECÁNICA - ELÉCTRICA - INSTRUMENTACIÓN',{relleno:azul,color:[255,255,255],negrita:true,centrado:true,tam:3.8});
      banda('1','DESCRIPCIÓN DEL AVANCE CONSTRUCTIVO',55);
      // Cronograma nativo simplificado: filas, fechas y barras dibujadas en el PDF.
      const tareas=['Permisos y difusión','Señalización e interferencias','Excavación y zanja','Pruebas de hermeticidad','Gasificación y cierre'];
      celda(14,60,88,27,'',{tam:3});celda(102,60,94,27,'',{tam:3});
      tareas.forEach((tarea,i)=>{const yy=61+i*5;doc.setDrawColor(180);doc.line(15,yy+4.4,101,yy+4.4);texto(`${i+1}. ${tarea}`,16,yy,34,4,{tam:2.7});doc.setDrawColor(110);doc.line(51,yy+2,98,yy+2);doc.setFillColor(i===3?80:225,i===3?160:120,i===3?200:90);doc.rect(53+i*5,yy+1.1,30-i*2,1.8,'F');});
      texto(detalle.avanceConstructivo||valor('supervisionObservaciones'),104,61,90,25,{tam:3.2});
      banda('2','RESUMEN EJECUTIVO - CONSTRUCCIÓN',87);
      const resumen=[['2.1','Permisos para la ejecución del proyecto',detalle.permisos||'Permisos de la fase en seguimiento'],['2.2','Recorrido informativo y difusión inicio de obra',detalle.recorrido||'—'],['2.3','Señalización y detección de interferencia',detalle.senalizacion||detalle.avanceConstructivo||'—'],['2.4','Excavación de calicatas',detalle.excavacion||'—']];
      resumen.forEach((fila,i)=>{const yy=92+i*6;celda(14,yy,11,6,fila[0],{centrado:true,negrita:true,tam:3.5});celda(25,yy,85,3,fila[1],{relleno:gris,color:[255,255,255],negrita:true,centrado:true,tam:3.2});celda(110,yy,86,3,'Comentarios',{relleno:gris,color:[255,255,255],negrita:true,centrado:true,tam:3.2});celda(25,yy+3,85,3,fila[2],{centrado:true,tam:3});celda(110,yy+3,86,3,'—',{centrado:true,tam:3});});
      celda(14,116,30,10,'3.1\nOBSERVACIONES\nAUDITORÍAS',{negrita:true,centrado:true,tam:3.2});celda(44,116,66,10,`RNC ABIERTAS        ${detalle.rncAbiertas||'0'}\nRNC CERRADAS       ${detalle.rncCerradas||'0'}\nTOTAL                    ${(Number(detalle.rncAbiertas)||0)+(Number(detalle.rncCerradas)||0)}`,{tam:3.5});celda(110,116,86,10,`RNC ABIERTAS        ${detalle.rncAbiertas||'0'}\nRNC CERRADAS       ${detalle.rncCerradas||'0'}\nTOTAL                    ${(Number(detalle.rncAbiertas)||0)+(Number(detalle.rncCerradas)||0)}`,{tam:3.5});
      celda(14,126,182,4,'Se recibió propuesta de acción correctiva y se mantiene el seguimiento de las RNC registradas.',{tam:3.3});
      banda('4','GESTIÓN DEL TIEMPO (CRONOGRAMA)',130);celda(14,135,182,19,detalle.gestionTiempo||valor('supervisionConclusiones'),{tam:3.5,negrita:true});
      banda('5','SEGURIDAD',154);celda(14,159,50,9,`5.1  CHARLA DE SEGURIDAD: ${detalle.charla||'—'}\nCAPACIDAD: —`,{tam:3.2});celda(64,159,32,9,`5.2  INCIDENTES: ${valor('supervisionIncidentes')||'0'}\nACCIDENTES: ${valor('supervisionAccidentes')||'0'}`,{tam:3.1});celda(96,159,100,9,`5.3  TEMA: ${detalle.temaSeguridad||'—'}`,{tam:3.4});
      banda('6','MANO DE OBRA - CONTRATISTA',168);
      ...
      celda(14,173,10,4,'ITEM',{relleno:azul,color:[255,255,255],centrado:true,negrita:true,tam:3.1});celda(24,173,60,4,'DESCRIPCIÓN',{relleno:azul,color:[255,255,255],centrado:true,negrita:true,tam:3.1});celda(84,173,18,4,'UNIDAD',{relleno:azul,color:[255,255,255],centrado:true,negrita:true,tam:3.1});celda(102,173,10,4,'CANT.',{relleno:azul,color:[255,255,255],centrado:true,negrita:true,tam:3.1});celda(112,173,84,4,'OBSERVACIONES',{relleno:azul,color:[255,255,255],centrado:true,negrita:true,tam:3.1});
      mano.forEach((fila,i)=>{const yy=177+i*3.2;celda(14,yy,10,3.2,fila[0],{centrado:true,tam:2.8});celda(24,yy,60,3.2,fila[1],{tam:2.8});celda(84,yy,18,3.2,'Und.',{centrado:true,tam:2.8});celda(102,yy,10,3.2,String(fila[2]),{centrado:true,tam:2.8});celda(112,yy,84,3.2,i===0?'Grupo de pruebas, hermeticidad y gasificación': '—',{tam:2.6});});
      celda(14,212.2,98,4,'Total',{negrita:true,centrado:true,tam:3.3});celda(102,212.2,10,4,String(personalTotal),{negrita:true,centrado:true,tam:3.3});celda(112,212.2,84,4,'Tubería instalada y zanja excavada: registro acumulado.',{tam:2.8});
      banda('7','NAGASCO - EQUIPOS',216.2);const equipos=[['1','RETROEXCAVADORA','0'],['2','CORTADORA','1'],['3','MINICARGADOR',minicargadores],['4','CANGURO','2'],['5','RODILLO','0']];celda(14,221.2,10,4,'ITEM',{relleno:azul,color:[255,255,255],centrado:true,negrita:true,tam:3.1});celda(24,221.2,60,4,'DESCRIPCIÓN DE EQUIPOS',{relleno:azul,color:[255,255,255],centrado:true,negrita:true,tam:3.1});celda(84,221.2,18,4,'UNIDAD',{relleno:azul,color:[255,255,255],centrado:true,negrita:true,tam:3.1});celda(102,221.2,10,4,'CANT.',{relleno:azul,color:[255,255,255],centrado:true,negrita:true,tam:3.1});celda(112,221.2,84,4,'OBSERVACIONES',{relleno:azul,color:[255,255,255],centrado:true,negrita:true,tam:3.1});equipos.forEach((fila,i)=>{const yy=225.2+i*3.6;celda(14,yy,10,3.6,fila[0],{centrado:true,tam:2.9});celda(24,yy,60,3.6,fila[1],{tam:2.9});celda(84,yy,18,3.6,'Und.',{centrado:true,tam:2.9});celda(102,yy,10,3.6,String(fila[2]),{centrado:true,tam:2.9});celda(112,yy,84,3.6,'—',{centrado:true,tam:2.9});});celda(14,243.2,98,4,'Total',{negrita:true,centrado:true,tam:3.3});celda(102,243.2,10,4,String(equiposTotal),{negrita:true,centrado:true,tam:3.3});celda(112,243.2,84,4,'—',{centrado:true,tam:3.1});
      banda('8','PERSONAL',247.2);celda(14,252.2,91,4,'SUPERVISIÓN END PERÚ S.A.C.',{relleno:azul,color:[255,255,255],centrado:true,negrita:true,tam:3.3});celda(105,252.2,91,4,'SUPERVISIÓN F.I.S.E.',{relleno:azul,color:[255,255,255],centrado:true,negrita:true,tam:3.3});celda(14,256.2,91,27,'',{tam:3});celda(105,256.2,91,27,'',{tam:3});doc.line(25,277,82,277);doc.line(116,277,184,277);texto(valor('supervisionSupervisor')||'Responsable de supervisión',25,278,57,4,{negrita:true,tam:3.2,centrado:true});texto('Representante FISE',116,278,68,4,{negrita:true,tam:3.2,centrado:true});
      // Dos páginas de fotografías, con imágenes extraídas de la plantilla real.
